...
namespace Client_LOURD
{
    public partial class ajoutReservation : Form
    {
        ...
        private void AjoutReservation_Load(object sender, EventArgs e)
        {
            ...
            this.cbSalle.DropDownStyle = ComboBoxStyle.DropDownList;
            this.cb_equipement.DropDownStyle = ComboBoxStyle.DropDownList;
        }
        ...
        private void btn_ajouter_reservation_Click(object sender, EventArgs e)
        {
           /* DateTime jour_res = dateTimePicker_jour.Value.Date;
            ...
            else
            {
                ...
                reservationList = reservationManager.getReservationDateHeureSalle(jour_res, heure_debut_res, heure_fin_res, id_salle, id_magasin);
                ...
                if (reservationList.Count == 0){
                    ...
                    string jour_res_new_format = jour_res.ToString("dd/MM/yyyy");
                    string heure_debut_res_new_format = heure_debut_res.ToString("HH:mm:ss");

                    if (dataGridView_equipements_choisis.Rows.Count != 0)
                    {
                        ...
                        for (int i = 0; i < dataGridView_equipements_choisis.Rows.Count; i++)
                        {
                            ...
                            int num_reservation = reservationManager.getId(jour_res_new_format, heure_debut_res_new_format, id_salle);

                            fournit unFournit = new fournit(0, id_equipement, num_reservation);
                            fournitManager fournitManager = new fournitManager();
                            fournitManager.addFournit(unFournit);

                            //MessageBox.Show(jour_res_new_format);
                            //MessageBox.Show(heure_debut_res_new_format);
                            //MessageBox.Show(Convert.ToString(id_salle));
                        }
                    }

                    //MessageBox.Show(Convert.ToString(reservationList.Count));
                    MessageBox.Show("La réservation a été ajouté avec succès");
                }
                else
                {
                    MessageBox.Show("Il existe déjà une réservation à cette horaire", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
        ...
        private void dataGridView_equipement_CellContentDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            int id = Convert.ToInt32(dataGridView_equipement.CurrentRow.Cells[0].Value);
